...
function Logo({ color = C.dark, size = 26 }) {
  return <span style={{ fontFamily: "'Caveat Brush', cursive", fontSize: size, color, letterSpacing: "0.02em", lineHeight: 1 }}>BADJR</span>;
}
  ...
  return (
    <>
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "space-between", padding: mobile ? "0 1.25rem" : "0 2.5rem", height: 60, background: scrolled || open ? "rgba(255,252,240,0.97)" : C.bg, borderBottom: `1px solid ${scrolled || open ? C.border : "transparent"}`, backdropFilter: scrolled || open ? "blur(12px)" : "none", transition: "border-color 0.3s, background 0.3s" }}>
        <a href="#top" style={{ textDecoration: "none" }} onClick={go}><Logo /></a>
        {!mobile && (
          <ul style={{ listStyle: "none", display: "flex", alignItems: "center", gap: "2rem", fontFamily: "'DM Sans', sans-serif" }}>
            {["About","Services","Projects","Team"].map(l => (
              <li key={l}><a href={`#${l.toLowerCase()}`} style={{ fontSize: "0.82rem", color: C.mid, textDecoration: "none", fontWeight: 400 }} onMouseEnter={e=>e.target.style.color=C.dark} onMouseLeave={e=>e.target.style.color=C.mid}>{l}</a></li>
            ))}
            <li><a href="#contact" style={{ fontSize: "0.82rem", fontWeight: 500, textDecoration: "none", padding: "0.45rem 1.1rem", background: C.green, color: C.white, fontFamily: "'DM Sans', sans-serif" }} onMouseEnter={e=>e.target.style.opacity="0.85"} onMouseLeave={e=>e.target.style.opacity="1"}>Contact</a></li>
          </ul>
        )}
        {mobile && (
      ...
      {mobile && open && (
        <div style={{ position: "fixed", top: 60, left: 0, right: 0, zIndex: 199, background: C.bg, borderBottom: `1px solid ${C.border}`, padding: "1.5rem 1.25rem 2rem", display: "flex", flexDirection: "column" }}>
          {["About","Services","Projects","Team"].map(l => (
            <a key={l} href={`#${l.toLowerCase()}`} onClick={go} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "1rem", color: C.dark, textDecoration: "none", padding: "0.9rem 0", borderBottom: `1px solid ${C.border}` }}>{l}</a>
          ))}
          <a href="#contact" onClick={go} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.9rem", fontWeight: 600, textDecoration: "none", padding: "0.85rem 1.25rem", background: C.green, color: C.white, marginTop: "1.25rem", textAlign: "center" }}>Contact us</a>
        </div>
      )}
    </>
  );
    ...
    <section id="top" style={{ background: C.bg, padding: mobile ? "100px 1.25rem 64px" : "140px 2.5rem 100px", maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ maxWidth: 780 }}>
        ...
        <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: mobile ? "2.6rem" : "clamp(2.8rem, 6vw, 5.2rem)", fontWeight: 400, color: C.dark, lineHeight: 1.1, letterSpacing: "-0.02em", marginBottom: "1.5rem" }}>
          We build software<br /><span style={{ color: C.green }}>that works beautifully.</span>
        </h1>
        ...
        <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
          <a href="#projects" style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 500, fontSize: "0.88rem", textDecoration: "none", padding: "0.7rem 1.5rem", background: C.dark, color: C.white }}>See our work</a>
          <a href="#contact" style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 400, fontSize: "0.88rem", textDecoration: "none", padding: "0.7rem 1.5rem", border: `1px solid ${C.border}`, color: C.dark }}>Start a project →</a>
        </div>
      </div>
    </section>
  );
...
function ProjectRow({ p, i, mobile }) {
  ...
  return (
    <div ref={r} style={{ ...fade(v, i*50), display: "grid", gridTemplateColumns: mobile ? "1fr" : "160px 1fr 1fr auto", alignItems: mobile ? "start" : "center", gap: mobile ? "0.3rem" : "2.5rem", borderBottom: `1px solid ${C.border}`, background: !mobile && hov && !dimmed ? C.bgAlt : "transparent", transition: "background 0.2s, opacity 0.6s ease, transform 0.6s ease", cursor: "default", margin: mobile ? "0" : "0 -2.5rem", padding: mobile ? "1.5rem 0" : "1.5rem 2.5rem", opacity: dimmed ? 0.45 : 1 }} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}>
      ...
      {!mobile && !dimmed && <a href="#contact" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.82rem", fontWeight: 500, color: hov ? C.green : C.border, textDecoration: "none", whiteSpace: "nowrap", transition: "color 0.2s" }}>View →</a>}
    ...
    <section id="projects" style={{ background: C.bg, padding: mobile ? "64px 1.25rem" : "100px 2.5rem" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div ref={cr} style={{ ...fade(cv), marginTop: "2.5rem", paddingTop: "2.5rem", borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
          ...
          <a href="#contact" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.82rem", fontWeight: 500, textDecoration: "none", padding: "0.6rem 1.3rem", background: C.green, color: C.white }}>Let's talk →</a>
        </div>
      </div>
    </section>
  );
...
function Contact() {
  ...
  const inputStyle = { fontFamily: "'DM Sans', sans-serif", fontSize: "0.9rem", color: C.dark, background: C.white, border: `1px solid ${C.border}`, padding: "0.7rem 0.9rem", outline: "none", width: "100%", borderRadius: 0, transition: "border-color 0.15s", WebkitAppearance: "none" };
  const labelStyle = { fontFamily: "'DM Sans', sans-serif", fontSize: "0.72rem", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 600, color: C.mid, display: "block", marginBottom: "0.5rem" };
  return (
    <section id="contact" style={{ background: C.bg, padding: mobile ? "64px 1.25rem" : "100px 2.5rem" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: mobile ? "3rem" : "7rem", alignItems: "start" }}>
        <div ref={r1} style={fade(v1)}>
          <Label>Contact</Label>
          <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", fontWeight: 400, color: C.dark, lineHeight: 1.2, letterSpacing: "-0.01em", marginBottom: "1.25rem" }}>Start a project</h2>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "1rem", lineHeight: 1.8, color: C.mid, marginBottom: "2rem" }}>We take on a limited number of projects at a time so we can focus fully on each one. Tell us what you're building — we'll get back to you within one business day.</p>
          {[["Email","info@badjrtech.com"],["Location","Remote-first · Worldwide"],["Response time","Within 24 hours"]].map(([k,val],i,arr) => (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "0.85rem 0", borderTop: `1px solid ${C.border}`, borderBottom: i===arr.length-1?`1px solid ${C.border}`:"none", gap: "1rem" }}>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.82rem", color: C.mid }}>{k}</span>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.82rem", color: C.dark, textAlign: "right" }}>{val}</span>
            </div>
          ))}
        </div>
        <div ref={r2} style={fade(v2, mobile ? 0 : 120)}>
          {done ? (
            <div style={{ padding: "2.5rem 0" }}>
              <div style={{ width: 36, height: 36, background: C.green, marginBottom: "1.25rem" }} />
              <h3 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "1.6rem", fontWeight: 400, color: C.dark, marginBottom: "0.5rem" }}>Message sent.</h3>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.9rem", color: C.mid }}>We'll be in touch within 24 hours.</p>
            </div>
          ) : (
            <form onSubmit={e=>{e.preventDefault();setDone(true);}} style={{ display: "flex", flexDirection: "column", gap: "1.1rem" }}>
              {[{l:"Name",n:"name",t:"text",ph:"Jane Smith"},{l:"Email",n:"email",t:"email",ph:"jane@company.com"}].map(f => (
                <div key={f.n}>
                  <label style={labelStyle}>{f.l}</label>
                  <input name={f.n} type={f.t} placeholder={f.ph} value={form[f.n]} onChange={ch} required style={inputStyle} onFocus={e=>e.target.style.borderColor=C.green} onBlur={e=>e.target.style.borderColor=C.border} />
                </div>
              ))}
              <div>
                <label style={labelStyle}>What do you need?</label>
                <select name="service" value={form.service} onChange={ch} required style={{...inputStyle, color: form.service ? C.dark : C.mid}}>
              ...
              <div>
                ...
                <textarea name="message" placeholder="What are you building? Any timeline or budget to share?" value={form.message} onChange={ch} required style={{...inputStyle, minHeight: 120, resize: "vertical"}} onFocus={e=>e.target.style.borderColor=C.green} onBlur={e=>e.target.style.borderColor=C.border} />
              </div>
              <button type="submit" style={{ background: C.dark, color: C.white, border: "none", padding: "0.8rem 1.75rem", fontFamily: "'DM Sans', sans-serif", fontSize: "0.88rem", fontWeight: 500, cursor: "pointer", alignSelf: "flex-start" }}>Send message</button>
            </form>
          )}
        </div>
      </div>
      ...
      <footer style={{ background: C.bg, padding: mobile ? "1.5rem 1.25rem" : "1.75rem 2.5rem", display: "flex", alignItems: mobile ? "flex-start" : "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem", flexDirection: mobile ? "column" : "row" }}>
        ...
        <ul style={{ listStyle: "none", display: "flex", gap: "1.5rem", flexWrap: "wrap" }}>
          {["About","Services","Projects","Team","Contact"].map(l => (
            <li key={l}><a href={`#${l.toLowerCase()}`} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.78rem", color: C.mid, textDecoration: "none" }}>{l}</a></li>
          ))}
        </ul>
      </footer>
    </>
